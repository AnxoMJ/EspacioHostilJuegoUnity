...
///Es la misma clase que la de Jugador, pero compatible con pc como dispositivos táctiles, haciendo uso de un plugin gratuito llamado simpleInput.
//El cual hace de intermediario y permite gestionar simultaneamente las dos entradas(teclado/mando y táctil)
public class JugadorAndroid_PC : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        vHorizontal = SimpleInput.GetAxis("Horizontal");
    ...
    void Update()
    {
        ...
        movInput = SimpleInput.GetAxisRaw("Horizontal");
        // Debug.Log(boton.);
        ...
        if (tocaSuelo)
            nSaltos = 0;
        if (((nSaltos < maxSaltos) || (tocaSuelo == true)) && (SimpleInput.GetButtonDown("Saltar")))
        {
            nSaltos++;
            estaSaltando = true;
            if (tocaSuelo)
        ...
        if (SimpleInput.GetButton("Saltar") && estaSaltando == true)
        {
            if (timerSalto > 0)
            {
                rb.velocity = Vector2.up * fuerzaSalto;
        ...
        if (SimpleInput.GetButtonUp("Saltar"))
        {
            estaSaltando = false;
        }
        animator.SetBool("Saltando", estaSaltando);
